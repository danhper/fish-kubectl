...
complete -c kubectl -x -l "as" -d "Username to impersonate for the operation"
...
complete -c kubectl -f -l "insecure-skip-tls-verify" -d "Certificate will not be checked for validity"
complete -c kubectl -r -l "kubeconfig" -d "Path to the kubeconfig file to use for CLI requests"
complete -c kubectl -x -l "log-backtrace-at" -d "Emit a stack trace when logging hits line file:N"
complete -c kubectl -r -l "log-dir" -d "Write log files in this directory"
complete -c kubectl -x -l "log-flush-frequency" -d "Maximum number of seconds between log flushes"
complete -c kubectl -f -l "logtostderr" -d "Log to standard error instead of files [default true]"
complete -c kubectl -f -l "match-server-version" -d "Require server version to match client version"
complete -c kubectl -x -s n -l "namespace" -d "The namespace scope for this CLI request"
complete -c kubectl -x -l "password" -d "Password for basic authentication to the API server"
complete -c kubectl -x -l "request-timeout" -d "Timeout for a single server request"
...
complete -c kubectl -f -l "vmodule" -d "List of settings for file-filtered logging"

complete -c kubectl -x -n "__kubectl_using_option -n --namespace" -a "(__kubectl_resources --no-prefix namespaces)" -d "Namespace"
complete -c kubectl -x -l "context" -a "(__kubectl_clusters)" -d "The name of the kubeconfig context to use"
complete -c kubectl -x -l "cluster" -a "(__kubectl_clusters)" -d "The name of the kubeconfig cluster to use"
